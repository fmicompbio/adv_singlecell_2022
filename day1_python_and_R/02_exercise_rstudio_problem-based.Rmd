...
you can also compile the whole document (`Knit` button on top).
...
- Is it using the same `python` process as the REPL above?
  Try to check this by printing one of the objects you created above

``````{comment}
```{python access_py_obj}
x
a
```
``````
...
- Add two python code chunks to your document.
- Create an object in the first one.
- Try to access that object in the second one.
...
## Add an R code chunk to your document that accesses python objects
...
## Add a `python` code chunk to your document that accesses `R` objects
- Maybe you need to first create some `R` objects in a separate chunk
- In your `python` chunk, use the special `r` object to access objects that live in
  the python process, for example `r$v`

The following steps are optional and further illustrate the object access:

- Let's first create numerical values drawn from normal
  distributions, one in R called `n1` and one in Python called `n2`:
...
- Let's calculate the mean of `n1` and `n2`, both in R and in Python:
``````{comment}
```{r, class.source="rchunk"}
mean(n1)
mean(py$n2)
...
- Round-trip: Write `iris` from `R` into a csv file, read it back into `pyiris2`
  in `python` and make available in `R` as `iris2` (note the data types of the
  "Species" column)
``````{comment}
```{r, class.source="rchunk"}
fname <- tempfile()
...
- Using the `iris` data frame, create a scatter plot of "Sepal.Length" versus
  "Petal.Length" with points colored by "Species", both in `R` and in `python`.
...
in memory, addressed by a single index (the memory address). Array memory
ordering schemes translate that single index into multiple indices corresponding
to the array coordinates. For example, matrices have two indices: rows and
columns. 
...
coordinate is the one changing fastest. To matrix element `x[i, j]`
(1-based indices) for example can be found at index `a = i + (j - 1) * nrow(x)`.
...
- From both environments, select and print values along just the first index of
  the first dimension (remember that indexing in `R` and `python` starts at 1
  0, respectively)
``````{comment}
```{r, class.source="rchunk"}
...
conversion will fail with a "cannot convert object" error.
...
- By default when `python` objects are returned to `R` they are converted to
  ...
  instance of an `R` reference class. See also the the `convert` argument of
  the `py_to_r()` function and 
  https://rstudio.github.io/reticulate/articles/calling_python.html#object-conversion
- `R` and `python` use different default numeric types - if `python` expects an
  integer, be sure to add L in R (e.g. `3L`).  
